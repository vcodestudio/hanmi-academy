<div class="row gap-32 page-wrap detail <?= 'type-'.get_post_type() ?>">
    ...
    <?php if($f=_acf("text-1")): ?>
        <hr/>
    <div class="gap-32 col col-2 tab-1">
        <h4 class="title" click="$('.tab-1').toggleClass('open')">
            목차
    </h4>
        <div class="gap-24 row">
            <div>
            ...
            // pdfs
                if($f=_acf("pdfs")):
                    ...
                    <hr/>
                    <div class="gap-8 row">
                    <strong>목차별 다운로드</strong>
                    <?php
                    foreach($f as $f_):
                        if(isset($f_["pdf"]) && $f_["pdf"]):
                        $item = $f_["pdf"];
            ?>
                <a href="<?= $item["url"] ?>" class="flex items-center gap-8 underline" download><?= $item["title"] ?></a>
            <?php
                        endif;
                    endforeach;
                    ?>
...
<script>
    ...
    //         // desc.insertAdjacentHTML('beforeend','<div>'+comp("more_down",["label"=>"자세히 보기"])+'</div>');
    //     }
    // });
</script>