...
if( function_exists('acf_add_local_field_group') ):
	...
	// [FAQ 필드그룹] 로컬(PHP) 정의를 제거했다 — DB 필드그룹(ID 16, 상태 publish, location page==181, repeater layout=block)이 SSOT.
	// 이 사이트에는 같은 key(group_63691aeadf746)의 DB 그룹이 존재하는데, 로컬로 재정의하면 웹 편집화면에서
	// 필드그룹이 아예 렌더되지 않아(the_content 에디터만 뜨고 FAQ 문항 repeater 미표시) 관리자가 내용을 확인/편집할 수 없었다
	// (acf_get_local_field_groups에서 이 key만 NOT_REGISTERED로 누락됨). 프론트(page-faq.php의 _acf("content"))와
	// 데이터(postmeta)는 DB 필드그룹으로 정상 동작한다. FAQ 필드 구성 변경은 ACF 관리화면(DB 그룹)에서 한다. (2026-07-22)
	...
	acf_add_local_field_group(array(
		...
		'fields' => array(
			...
			array(
				...
				'type' => 'url',
				'instructions' => '푸터에 표시될 인스타그램 링크 URL을 입력하세요.',
				'required' => 0,
				'conditional_logic' => 0,
				'wrapper' => array(
					'width' => '',
				...
				'default_value' => 'https://www.instagram.com/museumhanmi_academy',
				'placeholder' => 'https://www.instagram.com/museumhanmi_academy',
			),
			array(
...
지원 형식: MP4, WebM, MOV',
	...
	* 여러 장 등록 시 좌우로 슬라이드됩니다.',
			...
			array(
				...
				'name' => 'application',
				'type' => 'taxonomy',
				'instructions' => '신청중으로 설정하여도 상품정보 탭의 판매가능을 활성화하여야 실제 판매가 이루어집니다.',
				'required' => 0,
				'conditional_logic' => 0,
				'wrapper' => array(
					'width' => '',
